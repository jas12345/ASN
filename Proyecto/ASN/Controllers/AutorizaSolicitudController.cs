    ...
    public class AutorizaSolicitudController : Controller
    {
        ...
        public JsonResult GetPeriodoNominaCMB()
        {
            ...
            try
            {
                ...
                {
                    context.Database.CommandTimeout = int.Parse(ConfigurationManager.AppSettings["TimeOutMinutes"]);
                    listPeriodoNomina = context.CatPeriodosNominaCMB(0, usuario.UserInfo.Ident.Value,0).ToList();
                }
        ...
        public PartialViewResult GetEmpleadosDuplicados(int? FolioSolicitud)
        {
            try
            {
                using (ASNContext context = new ASNContext())
                {
                    context.Database.CommandTimeout = int.Parse(ConfigurationManager.AppSettings["TimeOutMinutes"]);
                    var lstSolicitudes = context.GetEmpleadosDuplicados(FolioSolicitud).ToList();
                    return PartialView("GetEmpleadosDuplicados", lstSolicitudes);
                }
            }
            catch (Exception ex)
            {
